...
icon_dir = r'Z:\Share\tcl0626\Maya_toolbox\plug-in\collect\破损插件 Damage Inc V 1.2\Icon'
icon_ext = '.png'
icon_size = 32
...
def MayaDamageUI(*args):
    ...
    cmds.text('            ')
    cmds.iconTextCheckBox('edgeDet', v=1, i=os.path.join(icon_dir, 'edgeDmgIcon' + icon_ext), w=icon_size, h=icon_size, ann='Damages the object along its edges', cc=partial(changeType, 'edge'))
    cmds.text('    ')
    cmds.iconTextCheckBox('surDet', v=0, i=os.path.join(icon_dir, 'surDmgIcon' + icon_ext), w=icon_size, h=icon_size, ann='Damages the object along its surface', cc=partial(changeType, 'surface'))
    cmds.text('    ')
    cmds.iconTextCheckBox('retopo', v=0, i=os.path.join(icon_dir, 'wireFIcon' + icon_ext), w=icon_size, h=icon_size, ann="Retopologizes the final mesh; doesn't keep the seperate material if applied")
    cmds.text('    ')
    cmds.iconTextCheckBox('keepOr', v=1, i=os.path.join(icon_dir, 'keepOrigIcon' + icon_ext), w=icon_size, h=icon_size, ann='Keeps original mesh and hides it')
    cmds.text('    ')
    cmds.iconTextCheckBox('apllyMat', v=1, i=os.path.join(icon_dir, 'applyMatIcon' + icon_ext), w=icon_size, h=icon_size, ann='Applies a separate material to the damaged area')
    cmds.text('    ')
    cmds.setParent('..')
    cmds.separator(h=20)
    cmds.rowColumnLayout(nr=1)
    cmds.text('        ')
    cmds.optionMenu('presetMenu', l='  Presets  ', w=180, cc=getPreset)
    ...
    cmds.text('        ')
    cmds.button(l='Save', ann='Save Current settings as preset', c=savePreset)
    cmds.text('    ')
    cmds.setParent('..')
    cmds.separator(h=20, style='none')
    cmds.rowColumnLayout(nr=1)
    cmds.text('      Preservation :          ')
    cmds.floatSliderGrp('StrengthValField', f=1, s=0.01, v=1.5, min=0.01, max=5, w=150, ann='Original Object Preservation (Reverse Strength)')
    cmds.setParent('..')
    cmds.separator(h=10, style='none')
    cmds.setParent('..')
    cmds.rowColumnLayout(nr=1)
    cmds.text('            Depth :              ')
    cmds.floatSliderGrp('DepthValField', f=1, s=0.01, v=0.5, min=0.01, max=2, w=150, ann='Depth')
    cmds.setParent('..')
    cmds.separator(h=10, style='none')
    cmds.setParent('..')
    cmds.rowColumnLayout(nr=1)
    cmds.text('      Noise Pattern :         ')
    cmds.intSliderGrp('PatternValField', f=1, v=1, min=0, max=4, w=150, ann='Noise Pattern. \n\n0 is Perlin Noise 3D \n\n1 is Billow 3D \n\n2 is Volume Wave 3D \n\n3 is Wispy 3D \n\n4 is Space Time 3D')
    cmds.setParent('..')
    cmds.separator(h=10, style='none')
    cmds.setParent('..')
    cmds.rowColumnLayout(nr=1)
    cmds.text('   Noise Frequency :      ')
    cmds.floatSliderGrp('FreqValField', f=1, s=0.001, v=1, min=0.001, max=1000.0, w=150, ann='Noise Frequency')
    cmds.setParent('..')
    cmds.separator(h=10, style='none')
    cmds.setParent('..')
    cmds.rowColumnLayout(nr=1)
    cmds.text('        Noise Detail :         ')
    cmds.floatSliderGrp('DetailValField', f=1, s=0.01, v=3.5, min=1, max=100.0, w=150, ann='Noise Detail')
    cmds.setParent('..')
    cmds.separator(h=10, style='none')
    cmds.setParent('..')
    cmds.rowColumnLayout(nr=1)
    cmds.text('        Noise Scale :          ')
    cmds.floatSliderGrp('NoiseScaleValField', f=1, v=1, min=0.1, max=100.0, w=150, ann='Noise Scale')
    cmds.setParent('..')
    cmds.separator(h=10, style='none')
    cmds.setParent('..')
    cmds.rowColumnLayout(nr=1)
