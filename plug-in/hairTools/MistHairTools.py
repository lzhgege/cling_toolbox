...
def hairballUI(*argvs):
    if mc.window(_ToolName, q=True, ex=True):
        mc.deleteUI(_ToolName)
    window = mc.window( _ToolName, title=u"迷雾Hairball v{}".format(_version).encode('GBK'), iconName='hairball', widthHeight=(200, 55) )

    mc.scrollLayout( 'scrollLayout' )
    mc.columnLayout( adjustableColumn=True )
    # mc.button(label='Reload', command=functools.partial(reload_window))
    ...
    mc.button( label='Cough it up!', command=functools.partial(makeHair, densityField, layerField, twistField))
    ...
    mc.button( label='Muss it up.', command=functools.partial(randomizeHair, rand1Field, rand2Field, rand3Field))
    ...
    mc.button( label='A little off the top.', command=functools.partial(trimHair, minTrimField, percentTrimField))
...
def reload_window(*argvs):
    hairballUI()

def makeHair(densityField, layerField, twistField, *argvs):
    firstLoop = mc.ls(sl=True, fl=True)
    density = mc.floatSliderGrp(densityField, q=True, value=True)
    layers = mc.intSliderGrp(layerField, q=True, v=True)
    twist = mc.floatSliderGrp(twistField, q=True, value=True)
...
def makeHullCurves(firstLoop, numIntermediates=0, *argvs):
...
def makeIntermediateCurves(curve1, curve2, numIntermediates=1, close=True, *argvs):
...
def orderEdgeloopVerts(verts, start=None, direction=None, *argvs):
	'''
...
def makeCurveFromVerts(verts, close=True, *argvs):
	p = []
	for vert in verts:
...
def makeCurveFromPoints(p, close=True, *argvs):
...
def getNeighboringEdgeloops(edgeLoop, *argvs):
	'''
...
def findCorrespondingVertInLoop(vert, edgeLoop, *argvs):
	'''
...
def makeHairCurves(hullCurves, d, twist = 0.0, *argvs):
	'''
	...
	for i in range(int(numCurves)):
		allCurves.append(makeHairCurve(hullCurves, i/(numCurves*1.0), twist))

	return allCurves

def makeHairCurve(hullCurves, u, twist=0.0, *argvs):
    '''
    ...
    icurve = mc.curve(p=p, d=1)
    icurve = mc.rebuildCurve(icurve, rpo=1, rt=0, end=1, kr=0, kcp=0, kep=1, kt=0, s=4, d=3, tol=0.000129167)
    mc.xform(icurve, centerPivots=True)
    
    return icurve

def randomizeHair(rand1Field, rand2Field, rand3Field, *argvs):
    ...
    curves = mc.ls(sl=True, fl=True, type='nurbsCurve', dag=True)
    rMult = [mc.floatSliderGrp(rand1Field, q=True, value=True), mc.floatSliderGrp(rand2Field, q=True, v=True), mc.floatSliderGrp(rand3Field, q=True, v=True)]
...
def trimHair(minTrimField, percentTrimField, *argvs):
    
    # random.randomly trim hair curves for more variation in length
    curves = mc.ls(sl=True, dag=True, type='nurbsCurve')
    min = mc.floatSliderGrp(minTrimField, q=True, value=True)
    percent = mc.floatSliderGrp(percentTrimField, q=True, value=True)
    ...
    for i in range(percentOfCurves):
        ...
        detach_list = mc.detachCurve('%s.u[%f]' % (activeCurve, r), ch=False, cos=True, rpo=True)
        
        mc.delete(detach_list[0])
        if len(detach_list) > 1 and not detach_list[1] in curves:
            curves.append(detach_list[1])
    mc.select(curves, r=True)
